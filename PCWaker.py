...
def next_time_start():
    # if today is value in Weekends[0] then get the first time range of Weekends[1], otherwise get the first time range of the next day
    if datetime.datetime.now().strftime('%A') == Weekends[0]:
        next_time = datetime.datetime.strptime(time_ranges[Weekends[1]][0][0],'%H:%M')
...
print_time("PCWaker reporting for duty")
while(True):
    check_if_skip()
    log_status()
    time.sleep(15)
